...
import { ArrowLeft, Flame, Loader2, Send, Sparkles, Info } from "lucide-react";
...
export default function ManagerEvaluationInput() {
  const { user } = useAuth();
  const [, params] = useRoute("/manager-evaluation/:userId");
  const [, setLocation] = useLocation();
  const targetUserId = params?.userId ? parseInt(params.userId) : 0;
  ...
  const submitMutation = trpc.evaluation.submitManagerEvaluation.useMutation();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      toast.success("評価を保存しました！", {
        description: "メンバーの成長を支援する評価が記録されました✨",
      });
      
      setTimeout(() => {
        setLocation("/manager-evaluation");
      }, 1500);
    } catch (error) {
      ...
      <div className="container max-w-4xl">
        ...
        <Card className="mb-8 border-orange-200 shadow-lg">
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <p className="text-gray-600">評価スコア</p>
              <div className="mt-4 flex items-center justify-center gap-2">
                <Sparkles className="w-5 h-5 text-orange-500 animate-float" />
                <span className="text-sm text-gray-500">
                  {totalScore} / {maxScore} ポイント
                </span>
              </div>
            </div>
          </CardContent>
        ...
        <Card className="mb-8 border-blue-200 shadow-md">
          <CardHeader className="bg-gradient-to-r from-blue-50 to-blue-100">
            ...
            <CardDescription>
              メンバーへの励ましのメッセージを添えましょう
            </CardDescription>
          </CardHeader>
          <CardContent className="pt-6">
            <Textarea
              placeholder="例：いつも前向きに取り組んでくれてありがとう！これからも一緒に成長していきましょう✨"
